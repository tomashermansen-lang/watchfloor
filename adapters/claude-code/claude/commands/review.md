...
If PLAN.md or TESTPLAN.md is missing, tell user to run `/plan` (it creates
both artifacts before review).
...
1. **Cross-document consistency.** Verify the full chain REQUIREMENTS → DESIGN → PLAN → TESTPLAN is consistent:
   ...
   - [ ] Every requirement in REQUIREMENTS.md has at least one scenario in TESTPLAN.md
   - [ ] Every component in PLAN.md has at least one scenario in TESTPLAN.md
   - [ ] TESTPLAN.md covers negative paths (errors, validation, boundaries) — not only golden paths
   ...
   - [ ] No TESTPLAN scenario that asserts behaviour outside the requirements (test creep)
...
6. **Verify TDD readiness and test plan quality.** Delegate to `test-explorer` subagent (Haiku, read-only) to check:
   - Can each component in PLAN.md be tested in isolation? What fixtures exist? What patterns are used in similar tests?
   - Does TESTPLAN.md cover every PLAN.md component and every REQUIREMENTS.md acceptance criterion?
   - Are negative-path scenarios specified (error handling, validation rejection, empty/boundary inputs)?
   - Are tautological or stub-friendly scenarios flagged ("test passes if function returns anything")?
   - Are integration boundaries (DB, network, IPC, filesystem) called out with the right test type?
   Findings about TESTPLAN quality enter the same review-fix loop as PLAN findings.
...
8. **Write review report.** Create `docs/INPROGRESS_Feature_<feature>/REVIEW.md`:
   ...
   ## Checklist
   - [ ] Cross-document consistency: REQUIREMENTS → PLAN → TESTPLAN
   - [ ] Feasibility against existing architecture
   - [ ] CLAUDE.md architecture rules
   - [ ] SOLID compliance
   - [ ] Agentic navigability
   - [ ] TDD readiness + test plan covers requirements and components
   ```
...
9. **Flow checkpoint.** Present using the Checkpoint Contract from the flow-mode skill:
   ...
   On [yes]:
     1. git add docs/INPROGRESS_Feature_<feature>/PLAN.md docs/INPROGRESS_Feature_<feature>/TESTPLAN.md docs/INPROGRESS_Feature_<feature>/REVIEW.md
...
- **NO TEST OR LINT RUNS.** This phase reads docs and modifies docs. Do
  not run pytest, vitest, ruff, mypy, eslint, tsc, shellcheck, or any
  other test suite, linter, or type-checker. Behavioral verification
  is `/implement`'s job; the `.tests-green-sha` marker is the downstream
  trust mechanism. If a reviewer subagent wants to "verify" something
  by running code, the right output is a finding (CRITICAL/WARNING)
  pointing the gap to `/implement` — not a tool invocation here.
